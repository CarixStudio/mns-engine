# Module 003 API
# CStructureEngine

Version: 1.0

---

## Purpose

Classifies confirmed market swing points and tracks trend and phase structure.

---

## Public Interface

- `bool Initialize(double minBreakDistance = 0.0)`
- `void Reset()`
- `bool Update(const CSwingDetector &detector, double currentAtr)`
- `SMarketState GetState() const`
- `bool IsBullish() const`
- `bool IsBearish() const`
- `bool IsTransition() const`
- `bool IsRanging() const`
- `double GetConfidenceScore() const`
- `string GetConfidenceInterpretation() const`
- `void SetOrderFlowAlignment(ENUM_MNS_ALIGNMENT val)`
- `void SetDisplacementQuality(ENUM_MNS_ALIGNMENT val)`
- `void SetMtfAgreement(ENUM_MNS_ALIGNMENT val)`
- `void SetActiveDeliveryAlignment(ENUM_MNS_ALIGNMENT val)`
- `void SetDolCompatibility(ENUM_MNS_ALIGNMENT val)`
- `void SetBosAlignment(ENUM_MNS_ALIGNMENT val)`
- `void SetInternalStructureAlignment(ENUM_MNS_ALIGNMENT val)`

---

## Private Methods

- `ENUM_MNS_STRUCTURE_TYPE ClassifyHigh(const SSwingPoint &current, const SSwingPoint &previous, double atrValue)`
- `ENUM_MNS_STRUCTURE_TYPE ClassifyLow(const SSwingPoint &current, const SSwingPoint &previous, double atrValue)`
- `void UpdateTrendAndPhase(const CSwingDetector &detector)`

---

## Private Members

- `bool m_isInitialized`
- `SMarketState m_state`
- `double m_minBreakDistance`
- `int m_lastProcessedExternalCount`
- `int m_lastProcessedInternalCount`

---

## Dependencies

- `MNSTypes.mqh`
- `CSwingDetector.mqh`
